...
class MatMulOp(Op):
    ...
    def compute(self, node, input_vals):
        ...
        """TODO: Your code here"""
        assert len(input_vals) == 2
        input_vals[0] = np.transpose(input_vals[0]) if node.matmul_attr_trans_A else input_vals[0]
        input_vals[1] = np.transpose(input_vals[1]) if node.matmul_attr_trans_B else input_vals[1]
    ...
    def run(self, feed_dict):
        ...
        for node in topo_order:
            if isinstance(node.op, PlaceholderOp):
                continue
            input_values = [node_to_val_map[_node] for _node in node.inputs]
            node_to_val_map[node] = node.op.compute(node, input_values)
...
def gradients(output_node, node_list):
    ...
    for node in reverse_topo_order:
        ...
        if len(node.inputs) == 0:
            continue
        grads_input = node.op.gradient(node, node_to_output_grad[node])
        for input_node in node.inputs:
            if input_node not in node_to_output_grads_list:
                node_to_output_grads_list[input_node] = []
        node_to_output_grads_list[node.inputs[0]].append(grads_input[0])
        if len(node.inputs) == 2:
            node_to_output_grads_list[node.inputs[1]].append(grads_input[1])
